...
SEMANTIC_COLORS = {
    "tint_primary": ("#0A84FF", "#0A84FF"),
    "tint_primary_hover": ("#0060DF", "#3694FF"),
    "tint_secondary": ("#30B0C7", "#5AC8F5"),
    "tint_secondary_hover": ("#1F8597", "#3AA9D6"),
    "text_primary": ("#000000", "#FFFFFF"),
    "text_secondary": ("#3C3C43", "#EBEBF5"),
    "text_inverted": ("#FFFFFF", "#0A0A0A"),
    "bg_base": ("#F2F2F7", "#000000"),
    "bg_container": ("#FFFFFF", "#1C1C1E"),
    "bg_secondary": ("#FFFFFFCC", "#2C2C2E"),
    "bg_grouped": ("#F2F2F7", "#1C1C1E"),
    "fill_card": ("#FFFFFF", "#2C2C2E"),
    "fill_subtle": ("#F8F8FA", "#2A2A2C"),
    "border_subtle": ("#D1D1D6", "#3A3A3C"),
    "danger": ("#FF3B30", "#FF453A"),
    "danger_hover": ("#D70015", "#FF6961"),
    "success": ("#34C759", "#32D74B"),
    "success_hover": ("#248A3D", "#30C447"),
    "muted": ("#8E8E93", "#98989D"),
}


def semantic_color(token: str) -> Tuple[str, str]:
    """Return a CustomTkinter-friendly color tuple for the given semantic token."""

    value = SEMANTIC_COLORS[token]
...
def current_color(token: str) -> str:
    """Return a concrete color string based on the current appearance mode."""

    mode = ctk.get_appearance_mode()
    light, dark = semantic_color(token)
    return light if mode == "Light" else dark


FONT_DISPLAY = ctk.CTkFont(family="SF Pro Display", size=24, weight="bold")
FONT_HEADLINE = ctk.CTkFont(family="SF Pro Text", size=18, weight="bold")
FONT_TITLE = ctk.CTkFont(family="SF Pro Text", size=16, weight="bold")
FONT_BODY = ctk.CTkFont(family="SF Pro Text", size=14)
FONT_CAPTION = ctk.CTkFont(family="SF Pro Text", size=12)
FONT_BUTTON = ctk.CTkFont(family="SF Pro Text", size=15, weight="bold")
FONT_MONO = ctk.CTkFont(family="SF Mono", size=12)


def apply_button_style(button: ctk.CTkButton, role: str = "primary") -> None:
    """Apply iOS-inspired button styling to the given button widget."""

    if role == "primary":
        button.configure(
            fg_color=semantic_color("tint_primary"),
            hover_color=semantic_color("tint_primary_hover"),
            text_color=semantic_color("text_inverted"),
            corner_radius=14,
            height=48,
            font=FONT_BUTTON,
        )
        ...
        button.configure(
            fg_color=semantic_color("fill_card"),
            hover_color=semantic_color("fill_subtle"),
            text_color=semantic_color("tint_primary"),
            border_color=semantic_color("border_subtle"),
            border_width=1,
            corner_radius=14,
            height=44,
            font=FONT_BUTTON,
        ...
        button.configure(
            fg_color=semantic_color("danger"),
            hover_color=semantic_color("danger_hover"),
            text_color=semantic_color("text_inverted"),
            corner_radius=14,
            height=48,
            font=FONT_BUTTON,
        )
    else:  # fallback to subtle style
        button.configure(
            fg_color=semantic_color("fill_subtle"),
            hover_color=semantic_color("fill_card"),
            text_color=semantic_color("text_primary"),
            corner_radius=12,
            height=44,
            font=FONT_BUTTON,
        )
...
class MemberEditDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent: ctk.CTk, member_data: Optional[Dict[str, str]] = None) -> None:
        ...
        self.grab_set()
        self.configure(fg_color=semantic_color("bg_grouped"))
    ...
    def _create_widgets(self) -> None:
        main_frame = ctk.CTkFrame(
            self,
            fg_color=semantic_color("bg_container"),
            corner_radius=20,
        )
        ...
        for label_text, placeholder in fields:
            ...
            label.pack(anchor="w", pady=(12, 4), padx=16)
            ...
            entry.pack(fill="x", pady=(0, 8), padx=16)
        ...
        active_checkbox = ctk.CTkCheckBox(
            ...
            text_color=semantic_color("text_primary"),
            border_color=semantic_color("border_subtle"),
            fg_color=semantic_color("tint_secondary"),
            hover_color=semantic_color("tint_secondary_hover"),
        )
        active_checkbox.pack(anchor="w", pady=(8, 16), padx=16)

        button_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
        button_frame.pack(fill="x", pady=(8, 0), padx=16)

        save_btn = ctk.CTkButton(button_frame, text="저장", command=self._on_save)
...
class MemberManagerDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self) -> None:
        ...
        buttons = [
            ("📁 엑셀 불러오기", self.import_excel, "secondary"),
            ("📥 샘플 템플릿", self.download_sample, "secondary"),
            ("➕ 회원사 추가", self.add_member, "primary"),
            ("✏️ 수정", self.edit_member, "secondary"),
            ("🗑️ 삭제", self.delete_member, "danger"),
            ("💾 저장", self.save_members, "primary"),
        ]

        for text, command, role in buttons:
            btn = ctk.CTkButton(button_frame, text=text, command=command, width=150)
        ...
        style.configure(
            ...
            font=("SF Pro Text", 12),
            rowheight=44,
            background=current_color("fill_card"),
            fieldbackground=current_color("fill_card"),
            foreground=current_color("text_primary"),
        )
        style.configure(
            "Treeview.Heading",
            font=("SF Pro Text", 12, "bold"),
            background=current_color("fill_subtle"),
            foreground=current_color("text_primary"),
        )
        style.map(
            "Treeview",
            background=[("selected", current_color("tint_primary"))],
            foreground=[("selected", current_color("text_inverted"))],
        )
        ...
        self.info_label = ctk.CTkLabel(
            ...
            text="",
            font=FONT_CAPTION,
            text_color=semantic_color("muted"),
        )
        self.info_label.pack(side="left", padx=16)

        close_btn = ctk.CTkButton(info_frame, text="닫기", command=self.destroy, width=120)
    ...
    def load_member_list(self) -> None:
        ...
        self.tree.tag_configure("inactive", foreground=current_color("muted"))
        self.update_info()
...
class SettingsDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self) -> None:
        ...
        tabview._segmented_button.configure(
            font=FONT_BODY,
            fg_color=semantic_color("fill_subtle"),
            selected_color=semantic_color("tint_primary"),
            selected_hover_color=semantic_color("tint_primary_hover"),
            unselected_color=semantic_color("fill_card"),
            text_color=semantic_color("text_primary"),
        )
        ...
        reset_btn = ctk.CTkButton(button_frame, text="기본값 복원", command=self.reset_settings)
        reset_btn.pack(side="left")
        apply_button_style(reset_btn, "secondary")
    ...
    def _create_crawling_settings(self, parent: ctk.CTkFrame) -> None:
        ...
        interval_frame.pack(fill="x", pady=12, padx=16)

        ctk.CTkLabel(interval_frame, text="요청 간격 (초)", font=FONT_TITLE).pack(anchor="w", pady=(16, 0), padx=16)
        self.interval_var = tk.IntVar(value=int(self.config.get("crawl_interval", 2)))
        self.interval_label = ctk.CTkLabel(interval_frame, text=f"{self.interval_var.get()}초", font=FONT_CAPTION, text_color=semantic_color("muted"))
        self.interval_label.pack(anchor="e", padx=16)
        ...
        self.interval_slider = ctk.CTkSlider(
            ...
            progress_color=semantic_color("tint_primary"),
        )
        self.interval_slider.pack(fill="x", pady=12, padx=16)
        self.interval_slider.set(self.interval_var.get())

        retry_frame = ctk.CTkFrame(
            parent,
            fg_color=semantic_color("fill_card"),
            corner_radius=18,
        )
        retry_frame.pack(fill="x", pady=12, padx=16)

        ctk.CTkLabel(retry_frame, text="재시도 횟수", font=FONT_TITLE).pack(anchor="w", pady=(16, 0), padx=16)
        self.retry_var = tk.IntVar(value=int(self.config.get("max_retries", 3)))
        retry_spin = tk.Spinbox(retry_frame, from_=1, to=10, textvariable=self.retry_var, width=6, font=("SF Pro Text", 12))
        retry_spin.pack(anchor="w", pady=12, padx=16)
        ...
        timeout_frame.pack(fill="x", pady=12, padx=16)

        ctk.CTkLabel(timeout_frame, text="요청 타임아웃 (초)", font=FONT_TITLE).pack(anchor="w", pady=(16, 0), padx=16)
        ...
        timeout_spin.pack(anchor="w", pady=12, padx=16)
        ...
        article_frame.pack(fill="x", pady=12, padx=16)

        ctk.CTkLabel(article_frame, text="회사당 최대 기사 수", font=FONT_TITLE).pack(anchor="w", pady=(16, 0), padx=16)
        ...
        self.articles_label.pack(anchor="e", padx=16)
        ...
        self.articles_slider.pack(fill="x", pady=12, padx=16)
    ...
    def _create_platform_settings(self, parent: ctk.CTkFrame) -> None:
        ...
        stored_platforms = set(self.config.get("platforms", ["google"]))
        enabled_platforms = {name for name in stored_platforms if name != "naver"}
        self.platform_vars: Dict[str, tk.BooleanVar] = {}

        platform_info = {
            "google": ("Google 뉴스", "구글에서 뉴스를 검색합니다."),
            "naver": ("Naver 뉴스", "현재 지원되지 않습니다."),
        ...
        for platform, (title, description) in platform_info.items():
            ...
            checkbox.pack(anchor="w", padx=16, pady=(16, 4))

            if platform in {"naver", "daum"}:
        ...
        option_frame.pack(fill="x", padx=16, pady=16)
        ...
        content_checkbox = ctk.CTkCheckBox(
            option_frame,
            text="기사 본문까지 수집 (현재 비활성화)",
            variable=self.content_var,
            state="disabled",
            font=FONT_BODY,
            text_color=semantic_color("muted"),
        )
        content_checkbox.pack(anchor="w", padx=16, pady=(4, 16))
    ...
    def _create_save_settings(self, parent: ctk.CTkFrame) -> None:
        ...
        auto_frame.pack(fill="x", padx=16, pady=12)
        ...
        auto_checkbox.pack(anchor="w", padx=16, pady=16)
        ...
        format_frame.pack(fill="x", padx=16, pady=12)

        ctk.CTkLabel(format_frame, text="기본 저장 형식", font=FONT_TITLE).pack(anchor="w", padx=16, pady=(16, 4))
        ...
        path_frame.pack(fill="x", padx=16, pady=12)

        ctk.CTkLabel(path_frame, text="저장 경로", font=FONT_TITLE).pack(anchor="w", padx=16, pady=(16, 4))
        self.path_label = ctk.CTkLabel(
            path_frame,
            text=str(self.selected_path),
            font=FONT_BODY,
            text_color=semantic_color("tint_primary"),
        )
        self.path_label.pack(anchor="w", padx=16, pady=(0, 12))

        change_btn = ctk.CTkButton(path_frame, text="경로 변경", command=self.change_save_path)
        change_btn.pack(anchor="w", padx=16, pady=(0, 16))
        apply_button_style(change_btn, "secondary")
    ...
    def save_settings(self) -> None:
        platforms = [name for name, var in self.platform_vars.items() if var.get() and name == "google"]
        if not platforms:
        ...
        self.config.set("enable_content_crawling", False)
...
class MainWindow(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        self.title("회원사 뉴스 수집 프로그램 v1.6.0")
        self.geometry("1024x760")
        self.minsize(920, 660)
        self.configure(fg_color=semantic_color("bg_base"))
    ...
    def _create_widgets(self) -> None:
        ...
        ctk.CTkLabel(
            title_frame,
            text="회원사 뉴스 수집을 더 쉽게, ver 1.6.0",
            font=FONT_BODY,
            text_color=semantic_color("muted"),
        ).pack(anchor="w", padx=28, pady=(0, 24))

        self._create_button_bar(title_frame)
    ...
    def _create_button_bar(self, parent: ctk.CTkFrame) -> None:
        ...
        buttons = [
            ("📋 회원사 관리", self.open_member_manager, "secondary"),
            ("▶️ 수집 실행", self.start_crawling, "primary"),
            ("📊 결과 보기", self.view_results, "secondary"),
            ("⚙️ 설정", self.open_settings, "secondary"),
            ("❓ 도움말", self.show_help, "secondary"),
        ]
    ...
    def _create_status_section(self, parent: ctk.CTkFrame) -> None:
        ...
        status_frame.pack(fill="x", pady=(24, 12))
        ...
        info_frame.pack(fill="x", padx=24, pady=(0, 24))
        ...
        for index, (key, label, default) in enumerate(items):
            ...
            name_label = ctk.CTkLabel(
                ...
                text=label,
                font=FONT_BODY,
                text_color=semantic_color("text_secondary"),
            )
            name_label.grid(row=row, column=col, sticky="w", padx=20, pady=18)
            ...
            value_label.grid(row=row, column=col + 1, sticky="w", padx=(0, 20), pady=18)
    ...
    def _create_action_section(self, parent: ctk.CTkFrame) -> None:
        ...
        action_frame.pack(fill="x", pady=(0, 24))
        ...
        apply_button_style(self.start_button, "primary")
        ...
        apply_button_style(self.stop_button, "danger")
        ...
        apply_button_style(self.export_button, "secondary")
        ...
        progress_frame.pack(fill="x", padx=24, pady=(8, 24))
        ...
        self.progress_label.pack(anchor="w", padx=20, pady=(20, 4))
    ...
    def _create_log_section(self, parent: ctk.CTkFrame) -> None:
        ...
        log_frame.pack(fill="both", expand=True, pady=24)
        ...
        clear_btn = ctk.CTkButton(header, text="지우기", width=100, command=self.clear_log)
        clear_btn.pack(side="right", padx=6)
        apply_button_style(clear_btn, "secondary")

        save_btn = ctk.CTkButton(header, text="로그 저장", width=120, command=self.save_log)
    ...
    def show_help(self) -> None:
        ...
        help_window.configure(fg_color=semantic_color("bg_container"))
